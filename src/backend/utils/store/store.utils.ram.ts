...
import { v4 } from 'uuid';
import type { CRUDStore, StoreTypes } from './store.utils.shared-models';

export const RAMStore = <Store>(options?: {
    searchLogic?: {
        list?: (
            entity: StoreTypes<Store>['details'],
            pattern: StoreTypes<Store>['findListPayload']
        ) => boolean;
        specific?: (
            entity: StoreTypes<Store>['details'],
            pattern: StoreTypes<Store>['findOnePayload']
        ) => boolean;
    };
    mappers?: {
        detailToList?: (
            details: StoreTypes<Store>['details']
        ) => StoreTypes<Store>['listModel'];
        createPayloadToDetail?: (
            details: StoreTypes<Store>['createPayload']
    ...
    class RAMStore implements StoreContract {
        ...
        protected mapDetailToList = (
            entity: StoreTypes<Store>['details']
        ): StoreTypes<Store>['listModel'] => {
            return entity as StoreTypes<Store>['listModel'];
        };

        protected mapCreatePayloadToDetail = (
            payload: StoreTypes<Store>['createPayload']
        ): StoreTypes<Store>['details'] => {
            return {
                ...(payload as object),
                id: v4()
        ...
        details: StoreContract['details'] = async (searchPayload) => {
            const details = this.data.find((x) =>
        ...
        updateOne: StoreContract['updateOne'] = async (
            searchPayload,
            updatePayload
        ) => {
