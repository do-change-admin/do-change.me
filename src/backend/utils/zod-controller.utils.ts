    ...
    const endpointLogic = <Method extends keyof T>(
        ...
        return async (request: NextRequest) => {
            ...
            try {
                ...
                const errorsWithSharedDetails: Codes[] = [
                    "Request parsing",
                    "Response parsing",
                ];
                ...
                return NextResponse.json<ErrorResponse>(
                    {
                        ...
                        details: errorsWithSharedDetails.includes(
                            controllerError.code as Codes
                        )
...
export function ZodController<T extends ZodControllerSchemas>(
    ...
    class ZODController<T extends ZodControllerSchemas> {
        ...
        protected logger = DIContainer()._context.get<ILoggerProvider>(
            ProviderTokens.logger
        );

        protected loggedEndpoint = zodAPIEndpointFactory(
            this.schemas,
            this.name,
            async ({ error }) => {
