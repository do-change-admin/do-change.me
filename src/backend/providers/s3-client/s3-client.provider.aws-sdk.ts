...
export class S3ClientAWSSDK implements S3Client {
    ...
    signedUploadLink: S3Client['signedUploadLink'] = async (bucket, key, fileType, originalName) => {
        const command = new PutObjectCommand({
            Bucket: bucket,
            Key: key,
            ContentType: fileType,
            ContentDisposition: originalName ? `attachment; filename="${originalName}"` : undefined
        })
