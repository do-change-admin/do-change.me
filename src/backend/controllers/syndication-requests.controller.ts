import {
    ZodAPIController,
    ZodControllerSchemas,
} from "@/backend/utils/zod-api-controller.utils";
import {
    type SyndicationRequestDraftsServiceFactory,
    type SyndicationRequestsServiceFactory,
} from "@/backend/di-containers/register-services";
...
import {
    SyndicationRequestsService,
    syndicationRequestsServiceSchemas,
} from "@/backend/services/syndication-requests.service";
...
const schemas = {
    ...
    PATCH: {
        body: undefined,
        query: syndicationRequestsServiceSchemas.addPhotos.payload.pick({
            id: true,
        }),
        response: undefined,
    },
...
@injectable()
export class SyndicationRequestsController extends ZodController(
    "Syndication requests",
    schemas
) {
    public constructor(
        @inject(ServiceTokens.syndicationRequestsFactory)
        private readonly serviceFactory: SyndicationRequestsServiceFactory,
        @inject(ServiceTokens.syndicationRequestDraftsFactory)
        private readonly draftsServiceFactory: SyndicationRequestDraftsServiceFactory
    ) {
        super();
    }
