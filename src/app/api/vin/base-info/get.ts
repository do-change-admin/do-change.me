...
const errorFactory = ErrorFactory.forController("vin / base-info");
...
export const method = zodApiMethod(schemas, {
    handler: async ({ payload, flags }) => {
        ...
        const apiAnswer = await fetch(
            `${process.env.BASE_INFO_API_URL}/${payload.vin}?format=json`
        );
        const json = await apiAnswer.json();

        if (!json?.Results?.[0]) {
            throw businessError(
                "No base data was found for this vehicle",
                undefined,
                404
            );
        }
    ...
    onSuccess: async ({ result, requestPayload, flags }) => {
        ...
        const isDemo = await isDemoVin({
            payload: { vin: requestPayload.vin },
        });
        if (!isDemo) {
