...
const errorFactory = ErrorFactory.forController("vin / salvage");
...
export const method = zodApiMethod(schemas, {
    handler: async ({ payload, flags }) => {
        ...
        const response = await fetch(
            `${process.env.SALVAGE_ENDPOINT!}?vin=${payload.vin}`,
            {
                method: "GET",
                headers: {
                    Referer: "rapidAPI",
    ...
    onSuccess: async ({ flags, result, requestPayload }) => {
        if (!flags[VinAPIFlags.DATA_WAS_TAKEN_FROM_CACHE]) {
            await prismaClient.salvageInfo.create({
                data: {
                    salvageWasFound: result.salvageWasFound,
